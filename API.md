...
### `GET /mock/admin/products` — 활성 종목 코드·이름 목록

인증: `x-admin-token` 헤더. DB `securities` 테이블(메인 서비스와 공유하는 종목 마스터)에서
`is_active=1`이고 시장구분이 코스피·코스닥인 종목을 코드순으로 돌려준다(코넥스는 뺀다 — 네이버
공개 시세 API가 코스피/코스닥만 지원). "과거 데이터 기반 시나리오 생성" 카드의 종목 선택창이
쓴다.
...
`prodCodes`를 비우면 DB `securities` 테이블의 활성 종목(코스피·코스닥) 전체를 기본 바스켓으로
쓴다 — 메인 서비스와 통일해서 쓰는 실제 종목 마스터라 이 목업 서버가 별도로 순위를 매기거나
목록을 들고 있을 필요가 없다. 그중 일부 종목의 과거 시세 조회가 실패하면(코드는 있지만 아직
네이버에 시세 이력이 없는 경우 등) 그 종목만 건너뛰고 계속 진행하며, 지정한 종목 전부 조회에
실패한 경우에만 `40001`이 난다.

하루마다: 그 종목이 그날 거래된 기록이 없으면(휴장일 등) 건너뛰고, 아니면 `buyProbability`
(기본 0.35)·`sellProbability`(기본 0.15)로 매수턴·매도턴이 뜰지 각각 독립적으로 정한다. 뜬 턴이
있으면 그날 종가를 매수·매도 가격으로 그대로 쓴다(단순화를 위해 종가로 통일). **매수턴을 먼저**
그 가격으로 살 수 있는 종목 일부를 사고, **그 다음 매도턴**이 보유 종목 일부를 판다. 매수 금액은
